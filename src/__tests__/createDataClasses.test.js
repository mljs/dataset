...
describe('createDataClasses', () => {
  ...
  it('should return dataClass in different formats', () => {
    ...
    expect(typeof dataClass.getDataClasses([0])[0].classFactor[0]).toEqual(
      'number',
    );
    expect(dataClass.getDataClasses([0])[0].classMatrix).toHaveLength(150);
    expect(typeof dataClass.getDataClasses([0])[0].classMatrix).toEqual(
      'object',
    );
  ...
  it('should return permuted dataClasses', () => {
    ...
    expect(
      dataClass.getPermutedClasses()[0].classFactor.reduce((a, c) => a + c),
    ).toEqual(150);
    expect(dataClass.getPermutedClasses()[0].classVector).not.toBe(
      dataClass.getDataClasses()[0].classVector,
    );
  });
  ...
  it('should return a smaller set without some observations', () => {
    ...
    dataClass.addDataClass('index', index);
    let newClass = dataClass.filterObservationsByIndex([
      0,
      1,
      2,
      12,
